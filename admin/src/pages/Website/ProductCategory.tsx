...
import { Button, Space, message, Modal, Form, Input, InputNumber, Popconfirm } from "antd";
import { EditOutlined, DeleteOutlined } from "@ant-design/icons";
import { TableList, type TableColumn } from "@/components/TableList";
import { getProductCategoryListApi, addProductCategoryApi, updateProductCategoryApi, deleteProductCategoryApi } from "@/api/website";
...
const ProductCategoryPage: React.FC = () => {
  ...
  const getList = async () => {
    setLoading(true);
    try {
      const res: any = await getProductCategoryListApi({ page: pagination.current, limit: pagination.pageSize });
  ...
  const handleDelete = async (id: number) => { try { await deleteProductCategoryApi(id); message.success("删除成功"); getList(); } catch { message.error("删除失败"); } };

  const handleSave = async () => {
    const values = await form.validateFields().catch(() => null);
    if (!values) return;
    try {
      if (isEdit && editId) { await updateProductCategoryApi({ id: editId, ...values }); message.success("更新成功"); }
      else { await addProductCategoryApi(values); message.success("新增成功"); }
    ...
    <>
      ...
      <Modal title={isEdit ? "编辑分类" : "新增分类"} open={modalVisible} onOk={handleSave} onCancel={() => setModalVisible(false)} width={400}>
        <Form form={form} layout="vertical" style={{ marginTop: 16 }}>
          ...
          <Form.Item name="slug" label="SEO URL" rules={[{ required: true }]}><Input placeholder="如：hardware" /></Form.Item>
